...
where the single-maintainer risk currently sits. It exists so that
users and potential contributors can make an informed judgement, and so the
maintainer has a written plan to act on.

It states the real situation rather than overclaiming: the gap below is open,
not solved.

## What reduces continuity risk today

These projects degrade gracefully even without active day-to-day attention:

- **Dependencies stay current automatically.** [Renovate](https://docs.renovatebot.com/)
  opens dependency and base-image update PRs and auto-merges the minor, patch,
  and digest ones on green CI, so the projects do not rot between releases.
- **Releases are fully automated and credential-light.** The release pipeline
  ...
  and publishes. Library publishing uses OIDC trusted publishers (npm/JSR);
  there are no long-lived publish tokens to lose or rotate.
...
- **Security scanning is continuous and unattended.** CodeQL and gitleaks run
  on every push to `main` and on every pull request; Trivy runs daily from a
  central schedule.
...
account-recovery and estate processes.
...
1. **Add a second maintainer.** Grant a trusted second person admin on the
   repositories, or move them to an organization with two owners, so issues can
   be triaged, changes merged, and releases cut by more than one person.
   Candidates: a developer family
